...
public class CustomInfiniteScrollAdapter<T extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<T>
        implements CustomDiscreteScrollLayoutManager.InitialPositionProvider {

    private static final int CENTER = Integer.MAX_VALUE / 2;
    private static final int RESET_BOUND = 100;

    public static <T extends RecyclerView.ViewHolder> CustomInfiniteScrollAdapter<T> wrap(
            @NonNull RecyclerView.Adapter<T> adapter) {
        return new CustomInfiniteScrollAdapter<>(adapter);
    }

    private RecyclerView.Adapter<T> wrapped;
    private CustomDiscreteScrollLayoutManager layoutManager;

    public CustomInfiniteScrollAdapter(@NonNull RecyclerView.Adapter<T> wrapped) {
    ...
    public void onAttachedToRecyclerView(@NonNull RecyclerView recyclerView) {
        ...
        if (recyclerView instanceof CustomDiscreteScrollView) {
            layoutManager = (CustomDiscreteScrollLayoutManager) recyclerView.getLayoutManager();
        } else {
