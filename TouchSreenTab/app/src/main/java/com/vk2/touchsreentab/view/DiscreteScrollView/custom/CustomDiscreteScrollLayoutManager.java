...
public class CustomDiscreteScrollLayoutManager extends RecyclerView.LayoutManager {
    ...
    private CustomDSVOrientation.CustomHelper orientationHelper;
    ...
    private boolean isGoToRight = true;
    private int side = 0;

    @NonNull
    private final CustomDiscreteScrollLayoutManager.ScrollStateListener scrollStateListener;
    private CustomScaleTransformer itemTransformer;

    private RecyclerViewProxy recyclerViewProxy;

    public CustomDiscreteScrollLayoutManager(
            @NonNull Context c,
            @NonNull CustomDiscreteScrollLayoutManager.ScrollStateListener scrollStateListener,
            @NonNull CustomDSVOrientation orientation) {
        this.context = c;
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        applyItemTransformToChildren(false);
    }
    ...
    protected void initChildDimensions(RecyclerView.Recycler recycler) {
        ...
        childHalfHeight = (int) (childViewHeight * getScaleHeight());
    ...
    protected void fill(RecyclerView.Recycler recycler) {
        ...
        layoutViews(recycler, CustomDirection.START, endBound);

        //Layout items after the current item
        layoutViews(recycler, CustomDirection.END, endBound);

        recycleDetachedViewsAndClearCache(recycler);
    }

    private void layoutViews(RecyclerView.Recycler recycler, CustomDirection direction, int endBound) {
...
//        Log.e("ScrollLayoutManager","layoutView position: "+position);
    ...
    protected int scrollBy(int amount, RecyclerView.Recycler recycler) {
        ...
        CustomDirection direction = CustomDirection.fromDelta(amount);
    ...
    protected void applyItemTransformToChildren(boolean isScrollEnd) {
        if (itemTransformer != null) {
            int clampAfterDistance = scrollToChangeCurrent * transformClampItemCount;
            float currentPositionView = getCurrentPositionView();
            if (currentPositionView < 0) {
                side = -1;
//                qua trái
                isGoToRight = false;
                Log.e("nhat", "qua trái");
            } else if (currentPositionView > 0) {
                side = 1;
//                qua phải
                isGoToRight = true;
                Log.e("nhat", "qua phải");
            ...
            if (isScrollEnd && side > 0) {
                for (int i = 0; i < 9; i++) {
                    View child = recyclerViewProxy.getChildAt(i);
                    itemTransformer.transformItemsRight(child, i);
                }
            } else if (side < 0) {
                for (int i = 0; i < 9; i++) {
                    View child = recyclerViewProxy.getChildAt(i);
                    itemTransformer.transformItemsLeft(child, i);
                }
            } else {
                for (int i = 0; i < recyclerViewProxy.getChildCount(); i++) {
                    View child = recyclerViewProxy.getChildAt(i);
                    itemTransformer.transformItem(child, Math.abs(currentPositionView), i, isGoToRight);
                }
            }
        }
    }
    ...
    private boolean onScrollEnd() {
        ...
        applyItemTransformToChildren(true);

        CustomDirection scrollDirection = CustomDirection.fromDelta(scrolled);
    ...
    private void onDragStart() {
        ...
        if (isAnotherItemCloserThanCurrent()) {
            CustomDirection direction = CustomDirection.fromDelta(scrolled);
    ...
    public void onFling(int velocityX, int velocityY) {
        ...
        int newPosition = currentPosition + CustomDirection.fromDelta(velocity).applyTo(throttleValue);
    ...
    protected int calculateAllowedScrollIn(CustomDirection direction) {
        if (pendingScroll != 0) {
        ...
        if (direction == CustomDirection.START && currentPosition == 0) {
        ...
        } else if (direction == CustomDirection.END && currentPosition == recyclerViewProxy.getItemCount() - 1) {
    ...
    private void startSmoothPendingScroll() {
        LinearSmoothScroller scroller = new CustomDiscreteScrollLayoutManager.DiscreteLinearSmoothScroller(context);
    ...
    private void startSmoothPendingScroll(int position) {
        ...
        pendingScroll = -scrolled;
        CustomDirection direction = CustomDirection.fromDelta(position - currentPosition);
    ...
    public void onAdapterChanged(RecyclerView.Adapter oldAdapter, RecyclerView.Adapter newAdapter) {
        pendingPosition = NO_POSITION;
        scrolled = pendingScroll = 0;
        if (newAdapter instanceof CustomDiscreteScrollLayoutManager.InitialPositionProvider) {
            currentPosition = ((CustomDiscreteScrollLayoutManager.InitialPositionProvider) newAdapter).getInitialPosition();
        } else {
            currentPosition = 0;
        }
        recyclerViewProxy.removeAllViews();
    ...
    public int getNextPosition() {
        if (scrolled == 0) {
            ...
            return currentPosition + CustomDirection.fromDelta(scrolled).applyTo(1);
        }
    }

    public void setItemTransformer(CustomScaleTransformer itemTransformer) {
...
//        return Math.min(Math.max(-1f, distanceFromCenter / maxDistance), 1f);
    ...
    private int getHowMuchIsLeftToScroll(int dx) {
        return CustomDirection.fromDelta(dx).applyTo(scrollToChangeCurrent - Math.abs(scrolled));
    }
    ...
    //    để biết đang kéo qua trái hay phải
    private float getCurrentPositionView() {
        float amountToScroll = pendingPosition != NO_POSITION ?
                Math.abs(scrolled + pendingScroll) :
                scrollToChangeCurrent;
        float position = -Math.min(Math.max(-1f, scrolled / amountToScroll), 1f);
    ...
    protected void setOrientationHelper(CustomDSVOrientation.CustomHelper orientationHelper) {
